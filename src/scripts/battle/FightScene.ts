...
export class FightScene {
  ...
  async initialize(locationName: string, fightInfo: IFightInfo): Promise<void> {
    ...
    // Pull ModelInfo from UserDataController instead of hardcoded strings.
    // Falls back to ModelInfo.createPlayer/createEnemy if controller isn't ready.
    const playerInfo = UserDataController.isReady
      ? UserDataController.getPlayerModelInfo()
      : undefined;
    const enemyInfo = UserDataController.isReady
      ? UserDataController.getDefaultEnemyModelInfo()
      : undefined;

    if (playerInfo && enemyInfo) {
      await modelsManager.createBattleModels(playerInfo, enemyInfo);
    } else {
      console.warn("[FightScene] UserDataController not ready, falling back to defaults");
      await Promise.all([
